  ...
  return (ctx, prev, draft) => {
    switch (ctx.type) {
      ...
      case A.ARRAY_REMOVE: {
        ...
        const base = draft.pendingFields ?? prev.pendingFields
        return { ...draft, pendingFields: reindexPathKeyedRecord(base, ctx.path, { type: 'remove', index: ctx.index }) }
      }
      case A.ARRAY_INSERT: {
        if (!ctx.path || ctx.index == null) return draft
        const base = draft.pendingFields ?? prev.pendingFields
        return { ...draft, pendingFields: reindexPathKeyedRecord(base, ctx.path, { type: 'insert', index: ctx.index }) }
      }
      case A.ARRAY_MOVE: {
        if (!ctx.path || ctx.from == null || ctx.to == null) return draft
        const base = draft.pendingFields ?? prev.pendingFields
        return { ...draft, pendingFields: reindexPathKeyedRecord(base, ctx.path, { type: 'move', from: ctx.from, to: ctx.to }) }
      }
      case A.ARRAY_SWAP: {
        if (!ctx.path || ctx.from == null || ctx.to == null) return draft
        const base = draft.pendingFields ?? prev.pendingFields
        return { ...draft, pendingFields: reindexPathKeyedRecord(base, ctx.path, { type: 'swap', from: ctx.from, to: ctx.to }) }
      }
      case A.RESET_FORM:
