...
export function valuesEnhancer<TValues>(defaultValues: TValues): Enhancer<TValues> {
  return (ctx, prev, draft) => {
    switch (ctx.type) {
      case A.SET_VALUE: {
        if (!ctx.path) return draft
        const base = draft.values ?? prev.values
        return { ...draft, values: setIn(base, ctx.path, ctx.value) as TValues }
      }
      case A.ARRAY_APPEND: {
        if (!ctx.path) return draft
        const base = draft.values ?? prev.values
        const arr = (getIn(base, ctx.path) as unknown[] | undefined) ?? []
        return { ...draft, values: setIn(base, ctx.path, [...arr, ctx.value]) as TValues }
      }
      case A.ARRAY_REMOVE: {
