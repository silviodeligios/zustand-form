...
export function validationEnhancer<TValues>(registry: FieldRegistry): Enhancer<TValues> {
  return (ctx, prev, draft) => {
    switch (ctx.type) {
      case A.SET_VALUE: {
        if (!ctx.path) return draft
        const entry = registry.get(ctx.path)
        if (!entry?.validate || (entry.validateMode ?? 'onChange') !== 'onChange') return draft
      ...
      case A.SET_ERROR: {
        if (!ctx.path) return draft
        const base = draft.errors ?? prev.errors
        return { ...draft, errors: { ...base, [ctx.path]: ctx.value as string } }
      }
      case A.CLEAR_ERROR: {
        if (!ctx.path) return draft
        const base = draft.errors ?? prev.errors
      ...
      case A.ARRAY_REMOVE: {
        ...
        const base = draft.errors ?? prev.errors
        return { ...draft, errors: reindexPathKeyedRecord(base, ctx.path, { type: 'remove', index: ctx.index }) }
      }
      case A.ARRAY_INSERT: {
        if (!ctx.path || ctx.index == null) return draft
        const base = draft.errors ?? prev.errors
        return { ...draft, errors: reindexPathKeyedRecord(base, ctx.path, { type: 'insert', index: ctx.index }) }
      }
      case A.ARRAY_MOVE: {
